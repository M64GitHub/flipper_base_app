...
typedef enum {
    ...
    M64AppScene_FirstLevel,
    M64AppScene_SecondLevel,
    M64AppScene_count
} M64AppScene;

/** ids for the 2 types of view used by the app */
typedef enum { M64AppView_Menu, M64AppView_Level } M64AppView;
...
/** all custom events */
typedef enum { M64AppEvent_ShowLevelOne, M64AppEvent_ShowLevelTwo } M64AppEvent;
...
void m64world_app_menu_callback_main_menu(void* context, uint32_t index) {
    ...
    case M64AppMenuSelection_One:
        scene_manager_handle_custom_event(app->scene_manager, M64AppEvent_ShowLevelOne);
        break;
    case M64AppMenuSelection_Two:
        scene_manager_handle_custom_event(app->scene_manager, M64AppEvent_ShowLevelTwo);
        break;
    }
}
...
bool m64world_app_scene_on_event_main_menu(void* context, SceneManagerEvent event) {
    ...
    case SceneManagerEventTypeCustom:
        switch(event.event) {
        case M64AppEvent_ShowLevelOne:
            scene_manager_next_scene(app->scene_manager, M64AppScene_FirstLevel);
            consumed = true;
            break;
        case M64AppEvent_ShowLevelTwo:
            scene_manager_next_scene(app->scene_manager, M64AppScene_SecondLevel);
            consumed = true;
            break;
        }
        break;
...
void m64world_app_scene_on_enter_level_one(void* context) {
    FURI_LOG_T(TAG, "m64world_app_scene_on_enter_level_one");
    M64App* app = context;
    popup_reset(app->popup);
    popup_set_context(app->popup, app);
    popup_set_header(app->popup, "BAM64!", 64, 1, AlignCenter, AlignTop);
    popup_set_icon(app->popup, 40, 35, &I_m64_logo);
    popup_set_text(app->popup, "Hello Mario64 World!", 20, 24, AlignLeft, AlignTop);
    view_dispatcher_switch_to_view(app->view_dispatcher, M64AppView_Level);
}

bool m64world_app_scene_on_event_level_one(void* context, SceneManagerEvent event) {
    FURI_LOG_T(TAG, "m64world_app_scene_on_event_level_one");
    UNUSED(context);
    UNUSED(event);
    return false; // don't handle any events
}

void m64world_app_scene_on_exit_level_one(void* context) {
    FURI_LOG_T(TAG, "m64world_app_scene_on_exit_level_one");
    M64App* app = context;
    popup_reset(app->popup);
}
...
void m64world_app_scene_on_enter_level_two(void* context) {
    FURI_LOG_T(TAG, "m64world_app_scene_on_enter_level_two");
    M64App* app = context;
    popup_reset(app->popup);
    popup_set_context(app->popup, app);
    popup_set_header(app->popup, "BAM64!", 64, 1, AlignCenter, AlignTop);
    popup_set_icon(app->popup, 40, 35, &I_m64_logo);
    popup_set_text(app->popup, "This is level 2!", 20, 24, AlignLeft, AlignTop);
    view_dispatcher_switch_to_view(app->view_dispatcher, M64AppView_Level);
}

bool m64world_app_scene_on_event_level_two(void* context, SceneManagerEvent event) {
    FURI_LOG_T(TAG, "m64world_app_scene_on_event_level_two");
    UNUSED(context);
    UNUSED(event);
    return false; // don't handle any events
}

void m64world_app_scene_on_exit_level_two(void* context) {
    FURI_LOG_T(TAG, "m64world_app_scene_on_exit_level_two");
    M64App* app = context;
    popup_reset(app->popup);
}
...
void (*const m64world_app_scene_on_enter_handlers[])(void*) = {
    ...
    m64world_app_scene_on_enter_level_one,
    m64world_app_scene_on_enter_level_two};
...
bool (*const m64world_app_scene_on_event_handlers[])(void*, SceneManagerEvent) = {
    ...
    m64world_app_scene_on_event_level_one,
    m64world_app_scene_on_event_level_two};

/** collection of all scene on exit handlers - in the same order as their enum */
void (*const m64world_app_scene_on_exit_handlers[])(void*) = {
    m64world_app_scene_on_exit_main_menu,
    m64world_app_scene_on_exit_level_one,
    m64world_app_scene_on_exit_level_two};
...
void m64world_app_view_dispatcher_init(M64App* app) {
    ...
    view_dispatcher_add_view(app->view_dispatcher, M64AppView_Level, popup_get_view(app->popup));
}
...
void m64world_app_free(M64App* app) {
    ...
    view_dispatcher_remove_view(app->view_dispatcher, M64AppView_Level);
